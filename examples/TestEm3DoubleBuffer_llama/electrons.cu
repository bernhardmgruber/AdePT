...
static __device__ __forceinline__ void TransportElectrons(const ParticleCount *electronsCount, View electrons,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < activeSize; i += blockDim.x * gridDim.x) {
    ...
    auto survive = [&] {
      ...
      if constexpr (IsElectron)
        secondaries.electrons.NextTrack() = currentTrack;
      else
        secondaries.positrons.NextTrack() = currentTrack;
    };
...
__global__ void TransportPositrons(const ParticleCount *electronsCount, View positrons, Secondaries secondaries,
                                   GlobalScoring *globalScoring, ScoringPerVolume *scoringPerVolume)
{
  TransportElectrons</*IsElectron*/ false>(electronsCount, positrons, secondaries, globalScoring, scoringPerVolume);
}
