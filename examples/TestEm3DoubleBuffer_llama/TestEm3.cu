...
struct Counts {
  ParticleCount *values[ParticleType::NumParticleTypes][2];
};

// Finish iteration: clear queues and fill statistics.
__global__ void FinishIteration(Counts counts, Stats *stats)
{
  for (int i = 0; i < ParticleType::NumParticleTypes; i++) {
    *counts.values[i][0] = 0;
    stats->inFlight[i]   = *counts.values[i][1];
  }
}
             ...
             GlobalScoring *globalScoring_host)
{
  ...
  for (int i = 0; i < ParticleType::NumParticleTypes; i++) {
    auto alloc = [](auto alignment, auto size) {
      std::byte *p = nullptr;
      COPCORE_CUDA_CHECK(cudaMalloc(&p, size));
      return p;
    };
    particles[i].tracksCurrent = llama::allocViewUninitialized(mapping, alloc);
    particles[i].tracksNext = llama::allocViewUninitialized(mapping, alloc);
  ...
  for (int startEvent = 1; startEvent <= numParticles; startEvent += batch) {
    ...
    do {
      ...
      const auto counts = Counts{{{particles[0].countCurrent, particles[0].countNext},
                                  {particles[1].countCurrent, particles[1].countNext},
                                  {particles[2].countCurrent, particles[2].countNext}}};
