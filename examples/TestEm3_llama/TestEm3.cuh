...
//   }
//   static __host__ __device__ __attribute__((noinline)) void FlatArrayWrapper(void *object, const int size, double
//   *vect)
//   {
...
struct RanluxppEngineImplLlama {
  VR vr;
  ...
  __host__ __device__ void Advance()
  {
    ...
    uint64_t state[9];
    SaveState(state);
    to_lcg(state, vr(Carry{}), lcg);
    mulmod(kA, lcg);
    to_ranlux(lcg, state, vr(Carry{}));
    LoadState(state);
    vr(BitPos{}) = 0;
  }
  ...
  __host__ __device__ uint64_t NextRandomBits()
  {
    ...
    boost::mp11::mp_with_index<9>(idx, [&](auto ic) {
      constexpr auto i = decltype(ic)::value;
      bits             = vr(State{}, llama::RecordCoord<i>{}) >> offset;
      if constexpr (i + 1 < 9) {
        if (numBits < w) {
          bits |= vr(State{}, llama::RecordCoord<i + 1>{}) << numBits;
        }
      }
    });
    bits &= ((uint64_t(1) << w) - 1);
...
struct RanluxppDoubleLlama final : public RanluxppEngineImplLlama<48, VR> {
  using Base = RanluxppEngineImplLlama<48, VR>;

  __host__ __device__ RanluxppDoubleLlama(VR vr /*, uint64_t seed = 314159265*/) : Base{vr}
  {
    //    this->SetSeed(seed);
  }

//  template <typename RD>
//  __host__ __device__ RanluxppDoubleLlama(llama::One<RD> vr /*, uint64_t seed = 314159265*/) : Base{vr}
//  {
//    static_assert(sizeof(RD) == 0, "Ask Bernhard");
//  }

  //  RanluxppDoubleLlama(const RanluxppDoubleLlama &other) : Base{std::forward<VR>(other.vr)} {}
  ...
  /// produced numbers.
  __host__ __device__ auto BranchNoAdvance()
  {
    ...
    RanluxppDoubleLlama<llama::One<RanluxDbl>> newRNG(this->vr);
    newRNG.XORstate(oldState);
    return newRNG;
  }

  /// Branch a new RNG state, also advancing the current one.
  __host__ __device__ auto Branch()
  {
    auto newRNG = BranchNoAdvance();
    newRNG.Advance();
    return newRNG;
  }
};

template <typename RanluxObj>
class RanluxppDoubleEngineLlama : public G4HepEmRandomEngine {
  // Wrapper functions to call into RanluxppDouble.
  static __host__ __device__ __attribute__((noinline)) double FlatWrapper(void *object)
  {
    return ((RanluxObj *)object)->Rndm();
  }
  static __host__ __device__ __attribute__((noinline)) void FlatArrayWrapper(void *object, const int size, double *vect)
  {
    for (int i = 0; i < size; i++) {
      vect[i] = ((RanluxObj *)object)->Rndm();
    }
  }

public:
  __host__ __device__ RanluxppDoubleEngineLlama(RanluxObj *engine)
      : G4HepEmRandomEngine(/*object=*/engine, &FlatWrapper, &FlatArrayWrapper)
  {
#ifdef __CUDA_ARCH__
    // This is a hack: The compiler cannot see that we're going to call the
