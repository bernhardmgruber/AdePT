             ...
             GlobalScoring *globalScoring_host)
{
  ...
  std::ofstream{"tracks.svg"} << llama::toSvg(
      Mapping{{32}}, 128); // global mem access is grouped in 32, 64 or 128 bytes.
  //  std::ofstream{"rng_sm.svg"} << llama::toSvg(RngStateMapping{});
  std::ofstream{"rng_sm.svg"} << llama::toSvg(
      llama::mapping::AoS<llama::ArrayExtents<int, TransportThreads>, RanluxDbl>{}, 4 * 32); // 32 banks of 4 bytes
